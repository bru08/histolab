...
class Tiler(Protocol):
    ...
    @lru_cache(maxsize=100)
    def tissue_mask(self, slide: Slide) -> sparse._coo.core.COO:
        """Return binary mask at level 0 of the box to consider for tiles extraction.

        The mask pixels set to True will be the ones corresponding to the tissue mask.
...
class RestrictedGridTiler(Tiler):
    """Extractor of tiles arranged in a grid, at the given level, with the given size.
        Tiles more restricted to the localized tissue area.
        Without using entire mask rescaling
    ...
    def _grid_coordinates_from_bbox_coordinates(
        ...
        Iterator[CoordinatePair]
            Iterator of tiles' CoordinatePair
        """
        self.tile_size_thumb = (
            int(self.tile_size[0] * slide.level_scale_factor(self.level) / slide.tissue_mask[1]),
            int(self.tile_size[1] * slide.level_scale_factor(self.level) / slide.tissue_mask[1])
            )

        tile_w_lvl, tile_h_lvl = self.tile_size_thumb
        #tile_w_lvl = tile_w_lvl // 2
        #tile_h_lvl = tile_h_lvl // 2
        ...
        for i in range(n_tiles_row):
            for j in range(n_tiles_column):
                ...
                # )
                # yield tile_wsi_coords
                res = CoordinatePair(x_ul_lvl, y_ul_lvl, x_br_lvl, y_br_lvl)
                yield res
    ...
    def _grid_coordinates_generator(self, slide: Slide) -> CoordinatePair:
        ...
        Iterator[CoordinatePair]
            Iterator of tiles' CoordinatePair
        """
        tissue_mask_small = self.tissue_mask(slide)[0]

        regions = regions_from_binary_mask(tissue_mask_small)
        for region in regions:  # at the moment there is only one region
            bbox_coordinates = region_coordinates(region)
            yield from self._grid_coordinates_from_bbox_coordinates(
                bbox_coordinates, slide
            )
        # TODO add middleman function taking grid coordiantes, cheking on thumb mask and thhen adapting coordinates
    ...
    def _grid_tiles_generator(self, slide: Slide) -> (Tile, CoordinatePair):
        ...
        grid_coordinates_generator = self._grid_coordinates_generator(slide)
        for coords in grid_coordinates_generator:
            # adapt coordinates from rescaled mask to level 0
            coords = scale_coordinates(
                reference_coords=coords,
                reference_size=slide.tissue_mask[0].shape[::-1],
                target_size=slide.level_dimensions(0)
            )
            try:
                tile = slide.extract_tile(coords, self.level)
            except ValueError:
    ...
    def _grid_plot(self, slide: Slide):
        """Generate diagnostic plot to visualize tiles selected

        """
        grid_coordinates_generator = self._grid_coordinates_generator(slide)
        thumb = np.copy(slide.resampled_array())
        thumb_msk = self.tissue_mask(slide)[0]
        down_factor = self.tissue_mask(slide)[1]
        l_width = max(1, (self.tile_size_thumb[0]//(10)))
        for x in grid_coordinates_generator:
            if np.mean(thumb_msk[x[1]:x[3], x[0]:x[2]]) > .5:
            
                thumb[x[1]:x[1]+l_width,x[0]:x[2],:] = (0.,255.,0.)
                thumb[x[3]-l_width:x[3],x[0]:x[2],:] = (0.,255,0.)
                thumb[x[1]:x[3],x[0]:x[0]+l_width,:] = (0.,255,0.)
                thumb[x[1]:x[3],x[2]-l_width:x[2],:] = (0.,255,0.)
        return thumb
    ...
    def _n_tiles_column(self, bbox_coordinates: CoordinatePair) -> int:
        ...
        """
        return (bbox_coordinates.y_br - bbox_coordinates.y_ul) // (
            self.tile_size_thumb[1] - self.pixel_overlap
        )
    ...
    def _n_tiles_row(self, bbox_coordinates: CoordinatePair) -> int:
        ...
        """
        return (bbox_coordinates.x_br - bbox_coordinates.x_ul) // (
            self.tile_size_thumb[0] - self.pixel_overlap
        )
